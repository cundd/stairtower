...
class IdentifierIndex extends Key {
	...
	public function indexDatabase($database) {
		...
		} else if ($database instanceof SplFixedArray) {
			$collection = $database;
...
//				DebugUtility::var_dump('Index', $key);
				...
				if (isset($tempMap[$key])) throw new DuplicateEntryException(sprintf('Duplicate entry \'%s\' for identifier', $key), 1415046937);
	...
	public function addEntryWithPosition($document, $position) {
		...
		if (isset($this->map[$key])) throw new DuplicateEntryException(sprintf('Duplicate entry \'%s\' for identifier', $key), 1415046937);
	...
	public function updateEntryForPosition($document, $position) {
		$key = DocumentUtility::getIdentifierForDocument($document);
		if (!isset($this->map[$key])) throw new InvalidEntryException(sprintf('Entry \'%s\' not found to update', $key), 1415047116);
		$this->map[$key] = $position;
		return $this;
	}
